...
RUN apt-get update && apt-get install -y postgresql-client-${POSTGRES_VERSION}

# Remove the apt-repo-setup helpers (only needed to add the pgdg repo above)
RUN apt-get remove -y curl gpg lsb-release && apt-get autoremove -y
